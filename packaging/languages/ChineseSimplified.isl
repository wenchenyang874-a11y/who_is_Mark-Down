...
ApplicationsFound=以下 WIMD 窗口正在使用需要更新的文件。选择“关闭 WIMD 并安装”后，未保存的 Markdown 正文会先写入本机临时恢复区，再关闭窗口并覆盖安装。安装完成页面默认勾选“重新打开并恢复 WIMD 窗口”；恢复后文档仍保持未保存状态，不会自动覆盖原文件。
ApplicationsFound2=以下 WIMD 窗口正在使用需要更新的文件。选择“关闭 WIMD 并安装”后，未保存的 Markdown 正文会先写入本机临时恢复区，再关闭窗口并覆盖安装。安装完成页面默认勾选“重新打开并恢复 WIMD 窗口”；恢复后文档仍保持未保存状态，不会自动覆盖原文件。
